...
def run():
    with sync_playwright() as p:
        ...
        responses=[]
        status={'service':'DrugIQ Atlas','ready':True}
        ...
        def route(req):
            url=req.request.url
            if url.startswith('https://drugiq.test/api/atlas'):
                if req.request.method=='GET': payload=status
                else:
                    data=req.request.post_data_json; responses.append(data)
        ...
        page.wait_for_function("!document.getElementById('atlas-connect').disabled")
        check('private form requires connection',page.locator('#variant-fields').evaluate('(e)=>e.disabled'))
        page.locator('#research-token').fill('test-only-password-'+ 'x'*32)
        page.locator('#atlas-connect').click()
        ...
        check('only intended fields sent',set(responses[-1])=={'action','assembly','chromosome','position','reference','alternate','scorer'})
        ...
        check('password never in exported result', 'test-only-password' not in page.locator('#source-details').inner_text())
        check('works without browser storage permissions',page.locator('#atlas-results').is_visible())
        ...
        check('clear removes password',page.locator('#research-token').input_value()=='')
        check('clear locks research form',page.locator('#variant-fields').evaluate('(e)=>e.disabled'))
        ...
        status['ready']=False
        page.close();page=context.new_page();load(page,PUBLIC/'variant-evidence.html');page.wait_for_timeout(200)
        check('unconfigured preview displays honest state','disabled' in page.locator('#atlas-status').inner_text())
        check('disabled API blocks UI',page.locator('#atlas-connect').is_disabled())
        browser.close()
    (REPORTS/'browser-checks.json').write_text(json.dumps({'passed':len(checks),'failed':0,'mode':'In-memory rendering; synthetic provider responses and blocked external services','checks':checks},indent=2))
